...
const bool BYPASS_AUTH = true;

final GoRouter router = GoRouter(
  initialLocation: BYPASS_AUTH ? '/dashboard' : '/login', // Start at dashboard in bypass mode
  refreshListenable: _authNotifier,
  redirect: (BuildContext context, GoRouterState state) async {
    // 🚀 BYPASS MODE: Skip all authentication checks
    if (BYPASS_AUTH) {
      print('⚠️ [Router] BYPASS MODE ACTIVE - Skipping authentication');
      return null; // Allow access to all routes
    }

    // Original authentication logic (only runs when BYPASS_AUTH = false)
    print('🔄 [Router] Checking route: ${state.matchedLocation}');
