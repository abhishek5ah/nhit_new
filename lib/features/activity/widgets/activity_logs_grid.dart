...
class ActivityGridView extends StatefulWidget {
  final List<ActivityLog> activityList;
...
class _ActivityGridViewState extends State<ActivityGridView> {
  ...
  Widget build(BuildContext context) {
    ...
    int end = (start + widget.rowsPerPage).clamp(0, widget.activityList.length);
    final paginatedLogs = widget.activityList.sublist(start, end);

    int totalPages = (widget.activityList.length / widget.rowsPerPage).ceil();

    // Pagination show 3  buttons
    int windowSize = 3;
    int startWindow = 0;
      ...
      children: [
                ...
                return MouseRegion(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      border: isHovered
                          ? Border(
                        top: BorderSide(color: logColor, width: 6),
                        left: BorderSide(color: logColor, width: 6),
                      )
                          : null,
                    ...
                    child: ProfileCard(
                      invoiceId: '#Log-${log.id.toString().padLeft(3, '0')}',
                      topBarColor: logColor,
                      fields: {
                        'Name': log.name,
                        'Description': log.description,
                        'Time Ago': log.timeAgo,
                      },
                    ),
                  ),
                );
        ...
        Padding(
          ...
          child: Row(
            ...
            children: [
              Text(
                "Showing ${widget.activityList.isEmpty ? 0 : start + 1} to $end of ${widget.activityList.length} entries",
