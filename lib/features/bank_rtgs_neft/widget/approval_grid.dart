...
class ApprovalRuleGridView extends StatefulWidget {
  ...
  @override
  State<ApprovalRuleGridView> createState() => _ApprovalRuleGridViewState();
}

class _ApprovalRuleGridViewState extends State<ApprovalRuleGridView> {
  ...
  Widget build(BuildContext context) {
    ...
    int end = (start + widget.rowsPerPage).clamp(0, widget.approvalRules.length);
    final paginatedRules = widget.approvalRules.sublist(start, end);

    int totalPages = (widget.approvalRules.length / widget.rowsPerPage).ceil();
              ...
              itemBuilder: (context, index) {
                ...
                return MouseRegion(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: ProfileCard(
                      invoiceId: '#Approver-${rule.id.toString().padLeft(3, '0')}',
                      topBarColor: roleColor,
                      fields: {
                        'Approver Level': rule.approverLevel,
                        'Users': rule.users,
                        'Payment Range': rule.paymentRange,
                      },
                    ),
                  ),
                );
